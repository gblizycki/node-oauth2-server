...
AuthorizeHandler.prototype.handle = function(request, response) {
  ...
  if (responseType === 'code') {
    var fns = [
      ...
      this.getClient(request, 'authorization_code'),
    ...
    var fns = [
      this.getClient(request, 'implicit'),
      this.getUser(request, response),
      this.getScope(request)
    ];
    return Promise.all(fns)
      .bind(this)
      .spread(function(client, user, scope) {
        ...
        return Promise.bind(this)
          ...
          .then(function(token) {
            var uri = this.getRedirectUri(request, client);
            return {token:token,redirectUri:uri};
          })
          .catch(function(e) {
            if (!(e instanceof OAuthError)) {
              e = new ServerError(e);
    ...
    throw new InvalidArgumentError('Invalid argument: `response_type` must be an code or token');
  }
...
AuthorizeHandler.prototype.getClient = function(request, grant) {
  ...
  return promisify(this.model.getClient, 2)(clientId, null)
    .then(function(client) {
      ...
      if (!_.contains(client.grants, grant)) {
      ...
      var deepContain = _.isEmpty(_.filter(client.redirectUris, function(item){
        return _.startsWith(redirectUri, item);
      }));
      if (redirectUri && !_.contains(client.redirectUris, redirectUri) && !deepContain) {
...
AuthorizeHandler.prototype.saveToken = function(user, client, scope) {
  var fns = [
...
AuthorizeHandler.prototype.generateAccessToken = function(client, user, scope) {
...
AuthorizeHandler.prototype.getAccessTokenExpiresAt = function() {
  var expires = new Date();
